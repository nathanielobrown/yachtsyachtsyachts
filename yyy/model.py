...
class Search(Base, DictMixin):
    ...
    search_time = Column(DateTime, default=datetime.datetime.now,
                        nullable=False)
...
if __name__ == '__main__':
    engine = create_engine('postgresql+psycopg2://postgres@localhost:5433',
    ...
    session = Session()
    s = Search(
        site_domain='www.boats.com',
        args=('contessa', 32),
        kwargs={})
    r = SearchResult(
        html='<h1>NOB</h1>',
        parsed_results={'title': 'NOB'}
    )
    s.results.append(r)
    session.add(s)
    session.commit()
