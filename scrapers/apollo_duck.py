...
class ApolloDuckScraper(BaseScraper):
    ...
    def search(self, manufacturer=None, length=None):
        ...
        manufacturer_ids = {make.lower().strip(): int(id.strip())
                            for id, make in matches}
        ...
        assert resp.status_code == 200
        matches = re.findall('/boats.phtml\?id=\d+&amp;mi=(\d+)">(.*?)<',
                             resp.content)
        model_ids = {model.lower().strip(): int(id.strip())
                     for id, model in matches}
